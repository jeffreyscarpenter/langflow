...
class NeMoDataPreparationComponent(Component):
    ...
    inputs = [
        ...
        DictInput(
            ...
            info="Custom field mappings as dictionary (e.g., {'prompt': 'input', 'completion': 'response'}). "
        ...
        IntInput(
            ...
            info="Number of records to randomly select and mark as evaluation data. "
            "Set to 0 to disable evaluation split.",
            value=20,
            required=False,
        ),
    ]
    ...
    def transform_to_chat_format(self, record: dict) -> dict:
        ...
        if self.preserve_unmapped_fields:
            ...
            result.update({key: value for key, value in record.items() if key not in mapped_fields})
    ...
    def process_data(self, input_data: list[Data] | pd.DataFrame) -> list[dict]:
        ...
        for i, record in enumerate(records):
            try:
                ...
                if (
                    not is_already_evaluation
                    and self.evaluation_records_count > 0
                    and evaluation_count < self.evaluation_records_count
                ):
        ...
        # Log statistics
        self.log(
            f"Data preparation completed: {valid_count} valid, {discarded_count} discarded, "
            f"{evaluation_count} evaluation records"
        )
