...
class TestNVIDIANeMoGuardrailsComponent:
    ...
    @pytest.mark.asyncio
    async def test_update_build_config_mode_wrapped(self, component):
        """Test build config update for wrapped mode."""
        build_config = {
            "model": {"show": False, "options": [], "value": ""},
            "max_tokens": {"show": False},
            "temperature": {"show": False},
            "top_p": {"show": False},
            "top_k": {"show": False},
            "seed": {"show": False},
            "frequency_penalty": {"show": False},
            "presence_penalty": {"show": False},
            "json_mode": {"show": False},
        }

        with patch.object(component, "fetch_guardrails_models", new_callable=AsyncMock) as mock_fetch_models:
            mock_fetch_models.return_value = ["model1", "model2"]

            await component.update_build_config(build_config, "wrapped", "mode")

            assert build_config["model"]["show"] is True
            assert build_config["model"]["options"] == ["model1", "model2"]
            assert build_config["model"]["value"] == "model1"
            assert build_config["max_tokens"]["show"] is True
            assert build_config["temperature"]["show"] is True
            assert build_config["top_p"]["show"] is True
            assert build_config["top_k"]["show"] is True
            assert build_config["seed"]["show"] is True
            assert build_config["frequency_penalty"]["show"] is True
            assert build_config["presence_penalty"]["show"] is True
            assert build_config["json_mode"]["show"] is True

    @pytest.mark.asyncio
    async def test_update_build_config_mode_check(self, component):
        """Test build config update for check mode."""
        build_config = {
            "model": {"show": True, "options": ["model1"], "value": "model1"},
            "max_tokens": {"show": True},
            "temperature": {"show": True},
            "top_p": {"show": True},
            "top_k": {"show": True},
            "seed": {"show": True},
            "frequency_penalty": {"show": True},
            "presence_penalty": {"show": True},
            "json_mode": {"show": True},
        }

        await component.update_build_config(build_config, "check", "mode")

        assert build_config["model"]["show"] is False
        assert build_config["model"]["value"] == ""
        assert build_config["max_tokens"]["show"] is False
        assert build_config["temperature"]["show"] is False
        assert build_config["top_p"]["show"] is False
        assert build_config["top_k"]["show"] is False
        assert build_config["seed"]["show"] is False
        assert build_config["frequency_penalty"]["show"] is False
        assert build_config["presence_penalty"]["show"] is False
        assert build_config["json_mode"]["show"] is False

    @pytest.mark.asyncio
    async def test_build_model_check_mode(self, component):
        """Test building model in check mode."""
        component.mode = "check"
        component.guardrails_config = "test_config"

        model = component.build_model()

        # Should return a CheckModeModel
        assert hasattr(model, "component")
        assert model.component == component

    @pytest.mark.asyncio
    async def test_check_mode_response_blocked(self, component):
        """Test check mode response when input is blocked."""
        component.mode = "check"
        component.guardrails_config = "test_config"
        component.input_value = "Test input"

        # Mock the guardrails check response
    ...
    @pytest.mark.asyncio
    async def test_check_mode_response_allowed(self, component):
        """Test check mode response when input is allowed."""
        component.mode = "check"
        component.guardrails_config = "test_config"
        component.input_value = "Test input"
